...
def module_completion_barplot(df):
    ...
    result = {}
    for module in modules:
        result[str(module)] = [round(get_completed_percentage(df, module, 'unlocked')*100, 1), 
                            round(get_completed_percentage(df, module, 'started')*100, 1),
                            round(get_completed_percentage(df, module, 'completed')*100, 1)]

    df_mod = pd.DataFrame(result, index = ['unlocked', 'started', 'completed']).T.reset_index().rename(columns={'index': 'Module'})

    # Melt the DataFrame to convert columns to rows
    melted_df = pd.melt(df_mod, id_vars='Module', value_vars=['unlocked', 'started', 'completed'],
                        var_name='Status', value_name='Percentage Completion')

    # Create a horizontal bar chart using Plotly
    fig_1 = px.bar(melted_df, y='Module', x='Percentage Completion', color='Status', orientation='h',
...
def module_completion_lineplot(df):
    ...
    for module, group in result_time.groupby('Module'):
        ...
        if len(sorted_group) == 1:
            fig_2.add_trace(go.Scatter(x=sorted_group['Date'], y=sorted_group['Percentage Completion'], mode='markers', name=module))
            
        else:
            fig_2.add_trace(go.Scatter(x=sorted_group['Date'], y=sorted_group['Percentage Completion'], mode='lines', name=module))
...
def item_completion_barplot(df):
    ...
    for module in modules:
        ...
        for i, item in enumerate(item_dict.get(module)):
            ...
            item_percent_completion = round((df_module_item[df_module_item['item_cp_req_completed'] == True]['student_id'].unique().size)*100/total_students, 0)
    ...
    fig_3 = make_subplots(rows=len(grouped_df), cols=1, shared_xaxes=True, vertical_spacing=0.01)
    ...
    for i, (module, group) in enumerate(grouped_df):
        ...
        fig_3.add_trace(go.Bar(y=group['Item'][::-1],
    ...
    fig_3.update_layout(height=150*len(grouped_df), title='Percentage Completion by Item for Each Module',
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    dcc.Graph(
                        id='plot2',
                        figure=module_completion_lineplot(data)
                    )
                )
            ],
            className="mt-4"
